...
function controleDeEventos(item, diaAnterior = '') {
  ...
  item.forEach(item => {
    if (item.situacao && controleDeFiltroEventos(item.data) && ftl.tipo != 1) {
      montaEventos(item)
    } else if (!item.situacao && controleDeFiltroEventos(item.data)) { // sit = false
      eventosFalsos.push(item)
    }else if(item.situacao && controleDeFiltroEventos(item.data) && ftl.tipo == 1){
      eventoDia.push(item)
    }
  ...
  if (itens.length) {
    ...
    itens.forEach(item => {
      let remove = `<div class='removeRecordEvent fadeIn' id='removeRecordEvent${item.key}' onclick="removedRecord('${item.key}','${item.data}')">
        <i class="fas fa-trash-alt"></i></div>`;
      eventosList.innerHTML += `
      <li class="list-group-item" style='text-decoration: line-through;' onmouseenter='eventoMouseEnterUl(this,1)' onmouseleave='eventoMouseLeaveUl(this,1)'>
      ${remove}
      <span class="spanTarefa">${item.data}</span> 
      <span class="spanTarefa" style='font-weight: bold;'>${item.hora} Hrs</span> 
      <span>${item.descricao}</span>
...
function arrayDeEventosEmAndamento() {
  dadosEventos.forEach(item => {
    if (item.situacao && item.data.substr(3, 2) == mesAtual && item.data.substr(6)) {
...
function MainContagemEventosMes(diaAnterior) {
  ...
  dadosEventos.forEach(item => {
    if (item.situacao && item.data.substr(3, 2) == mesAtual && item.data.substr(6) == anoAtual) {
      var dia = item.data
      var [verifica, cont, diaAnt] = contarEventosNoDia(diaAnterior, dia, contagem[0])
      contagem[0] = cont
      diaAnterior = diaAnt
